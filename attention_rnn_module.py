...
def define_model(vocab_size, rnn_dimension=128, max_length, loss="binary_crossentropy", attention_layers=[128,128,128]):
	...
	att = keras.layers.wrappers.TimeDistributed(Dense(1, activation=None))(att if len(attention_layers)>0 else rnn)
	att = keras.layers.Flatten()(att)
	att = keras.layers.Activation('softmax')(att)
	att = keras.layers.core.RepeatVector(rnn_dimension)(att)
	att = keras.layers.core.Permute([2, 1])(att)
	arnn = keras.layers.merge([att, rnn], mode='mul')
	arnn = Lambda(lambda x: K.sum(x, axis=1))(arnn)
	# EndOf ATTENTION
	fnn = Dense(1, activation='sigmoid')(arnn)
