...
if __name__ == "main":
    ...
    data = pd.merge(left=notes[["SUBJECT_ID", "CATEGORY", "TEXT"]], right=patients[["SUBJECT_ID", "GENDER"]],
    ...
    if use_gender_split:
        ...
        data = pd.concat([data_m.sample(train_size * 5), data_f.sample(train_size * 5)]).sample(frac=1).reset_index(
            drop=True)
